...
const configSchema = Joi.object({
    ...
    connectionAuth: Joi.alternatives().try([
        Joi.func().description(
            'function with signature (req) => { return true/false; } to check connection auth. e.g. Basi Auth'
        ),
        Joi.object({
        ...
        if (this.config.connectionAuth) {
            if (typeof this.config.connectionAuth === 'function') {
                ...
                soapServer.authorizeConnection = req => {
                    ...
                    return (
                        credential.name === this.config.connectionAuth.basicAuth.username &&
                        credential.pass === this.config.connectionAuth.basicAuth.password
                    );
                };
            }
        }
        ...
        files
            ...
            .forEach(f => {
                ...
                svc[service][port][method] = (args, callback, headers, req) => {
                    this.log.debug(`Execute handler: ${this.config.path}/${service}/${port}/${method}`, args, headers);
                    const reqId = uuid();
                    const request = {
                    ...
                    handlerFn(
                        ...
                        .catch(e => {
                            this.log.error(
                                `ERROR: in handler: ${this.config.path}/${service}/${port}/${method}`,
                                args,
                                headers,
                                e,
                                e.stack
                ...
                this.log.debug(`Setup SOAP handler for: ${this.config.path}/${service}/${port}/${method}`);
            });
