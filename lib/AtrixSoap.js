...
const configSchema = Joi.object({
	enabled: Joi.boolean().default(true).description('whether the endpoint should be started'),
			...
			svc[service][port][method] = (args, callback, headers, req) => {
				...
				handlerFn(request, (res) => { result = res; }, this.service)
					.then(() => {
						...
						this.log.error(`ERROR: in handler: ${this.config.path}/${service}/${port}/${method}`, args, headers, e, e.stack);
